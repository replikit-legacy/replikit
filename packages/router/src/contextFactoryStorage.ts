...
type ContextFactory<T extends EventName = EventName> = (
    event: Event<T>
) => ContextMap[T];
...
export class ContextFactoryStorage {
     ...
     */
    async createContext<T extends EventName>(
        event: Event<T>
    ): Promise<ContextMap[T]> {
        const factory = this.factoryMap.get(event.type);
        if (!factory) {
            throw new Error(
                `Context factory for event ${event.type} not found`
            );
        }
